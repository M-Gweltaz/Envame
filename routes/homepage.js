...
router.post('/newsletter', async (req, res) => {
  ...
  let mailOption = {
    from: '"Envame newsletter" <gweltaz.dev@laposte.net>', // sender address
  ...
  const regex = /^([\w-\.éèëêàäâïîüûö<]{2,40})@((?:[\w]{2,25}\.))([a-zA-Z]{2,4})/i
  let fetchResult 
  
  switch (regex.test(data.newsletterEmail)) {
    case true:
      fetchResult = 'success'
...
router.put('/newsletter_unsubscribe', async (req, res) => {
  const data = req.body
  let deleteResult 
